...
本文把这条链路讲透。人物、武器、光效、粒子会作为贯穿例子出现；最后补充 WebGPU / WebGL 一类图形 API 里 UI 与文字是怎么画出来的。配套可运行演示见 [`webgpu-shader-demo`](https://github.com/shenjunjian/wasm-road-blog/tree/main/webgpu-shader-demo/)。浏览器以 Chrome / Edge 的 WebGPU 支持最为稳定。
...
| 层次 | 指什么 | 会不会并行 |
|---|---|---|
| **命令顺序** | CommandBuffer 里各次 `draw` / `drawIndexed` 的先后 | **不会**；GPU 按录制顺序依次发起每一笔 draw |
| **流水线并行** | 单笔 draw 内的顶点/片元；或多笔 draw 占用流水线的不同阶段 | **会**；大量 shader core 同时工作，各阶段在时间上重叠 |

因此：

- **批次与批次之间**：上一批次的全部 draw 在输出语义上收尾后，才能 `setPipeline` 换到下一批（例如从不透明换到半透明）。
- **同一批次内的多次 draw**：命令上**依次**执行（草地 → 人物 → 武器），**不是**三个 draw 同时起跑；每一笔 draw 仍各自完整走 VS → 光栅化 → FS → 深度/混合，只是共用同一套管线状态，不必重复切换 shader 与固定功能配置。
- **同一 draw 内部**：VS、光栅化、FS 在硬件流水线上重叠推进，不是「先算完所有顶点再统一光栅化」。

「合批」省的是**管线状态切换**成本，不是把多次 draw 合并成一次（§2.2 展开）；GPU 如何在遵守命令顺序的前提下做流水线并行，见 §3.7。
...
#### 不透明批次：一次 `setPipeline`，多次 `drawIndexed`
...
pass.drawIndexed(6);                // 草地：4 顶点、6 索引 = 2 三角形
...
上面三次 `drawIndexed` 共用 `opaquePipeline`，只换了 BindGroup / 顶点缓冲 / 索引缓冲——这就是**合批**：管线状态（shader、深度写入、混合）相同，就不要为每个物体重复 `setPipeline`。
...
| 省下来的 | 没省下来的 |
|---|---|
| 不必重复 `setPipeline`（shader、深度写入、混合不变） | 三次独立的完整渲染流程 |
| `setBindGroup` / `setVertexBuffer` / `setIndexBuffer` 切换成本相对低 | 各物体的顶点数、片元数照常各自计算 |

记法：**管线状态共享，绘制次数不合并**。每一次 `drawIndexed`，GPU 仍对该笔几何完整走 VS → 光栅化 → FS → 深度测试 → 混合（不透明时混合等价于覆盖）。三笔 draw 之间变的是「绑哪组 Uniform、用哪份顶点/索引」；不变的是「哪套 shader、开不开深度写、开不开混合」。

**② 三次 draw 是同时跑，还是必须排队？**

- **CPU 侧**：在 `CommandEncoder` 里**按顺序录制**三条 draw 命令，然后一次 `submit`；录制本身几乎瞬间完成，JS 不会等 GPU 画完草地再录人物。
- **GPU 侧**：收到 CommandBuffer 后，**按录制顺序依次执行**每一笔 `drawIndexed`——先草地、再人物、再武器。它们**不是**三个互不相关的任务被同时丢进 GPU 并行起跑。
- **为什么命令必须有序**：深度缓冲与混合都依赖「谁先写进帧缓冲」。草地先写深度，人物、武器再做深度测试，遮挡才正确；若乱序，可见性会错。
- **并行发生在哪里**：不在「哪笔 draw 先发起」上，而在**每一笔 draw 内部**——GPU 用大量 shader core 并行处理不同顶点、不同片元。此外，硬件流水线常让 draw A 的片元阶段与 draw B 的顶点阶段在**时间上重叠**（§3.7），但这不改变 API 语义：争用同一像素时，输出合并仍须与命令录制顺序一致。

**③ 演示规模下 GPU 为什么看起来「大材小用」**

配套 demo 里草地只是 **4 顶点、2 三角形** 的大平面；人物、武器网格也不大，整帧 opaque 批次只有三次 draw。对现代 GPU 来说，这点几何远填不满并行单元——**利用率低是预期现象**，不是合批失败或 API 用错。

教学 demo 故意拆成三次 draw，是为了看清换 BindGroup / VB / IB 的写法；工业场景才会合并网格、Instancing、Multi-draw 去喂饱 GPU。对本 demo，帧率瓶颈几乎不在 GPU 算力，而在 vsync 或浏览器合成。
...
3. **同一 `opaquePipeline` 下连续三次 `drawIndexed` 是合批**：只换 BindGroup / VB / IB，不重复换管线；但 GPU 仍按录制顺序依次执行三笔 draw，不是同时起跑（§2.2）。  
4. **`submit` 只提交指令清单（普通内存）**，GPU 按清单去显存里取 VB/Uniform/管线执行。
...
CPU 在 §2.2 里已按顺序录好命令；GPU 取出 CommandBuffer 后，在 Render Pass 内部大致等价于：

```text
for 每一个「管线状态段 / 批次」:
    加载该批次的 VS、FS、深度与混合配置（setPipeline 的结果）
    for 该批次内每一次 draw（严格按录制顺序）:
        绑定本次 draw 的资源（BindGroup / VB / IB）
        发起 draw → 该笔几何进入图形流水线
        // 命令顺序：draw 与 draw 之间依次发起，不会同时起跑
        // 流水线并行：不同 draw 可占用 VS / 光栅 / FS 的不同阶段（§3.7）
        // 输出合并：争用同一像素时，深度 / 模板 / 混合 / 写回按 draw 顺序串行化
    本批次全部 draw 完成后，才 setPipeline 换下一批
```

对演示场景，GPU **按录制顺序**依次执行各笔 draw。前三笔同属 opaque 批次（不换管线）：

| 顺序 | draw 内容 | 是否换管线 |
|---|---|---|
| 1 | 草地（2 三角形） | 否（opaque 批次开始） |
| 2 | 人物 | 否 |
| 3 | 武器 | 否 |
| 4 | 法术环（可多圈） | 是 → auraPipeline |
| 5 | 粒子实例 | 是 → particlePipeline |

前三笔共用 opaque 管线，只换了 VB / IB / Uniform，切换成本低于「每物体一套不同 WGSL」。法术环与粒子要换混合状态（加色混合、关闭 depth write），属于新批次。

**批次之间**：上一批全部 draw 完成后，才安全地换管线开下一批。  
**同一批次内的多次 draw**：命令**依次**发起；流水线阶段可重叠，但争用同一像素/采样点时，输出合并须与录制顺序一致（§3.7）。
...
### 3.7 单批次内部：命令有序，流水线并行

§2.2 已说明：同一批次内的多次 draw **按录制顺序依次发起**，不是同时起跑。这里补硬件侧细节——**发起有序**不等于**整段流水线串行掏空**。

在同一批次、同一套管线里，硬件像工厂传送带：

1. draw A 的一部分顶点还在跑 VS；  
2. draw A 更早完成的三角形已在光栅化；  
3. draw A 的片元已在 FS，或已进入输出合并；  
4. 同时，draw B 的顶点可能刚进入 VS——**与 draw A 的 FS 阶段时间上重叠**。

这不是：

- 错法 A：draw A 完整 VS→…→混合 全部结束，再启动 draw B（按 draw 串行掏空流水线）；  
- 错法 B：三个 draw 同时起跑、互不相关（违反 CommandBuffer 顺序语义）；  
- 错法 C：先把本帧所有物体顶点全部 VS 完，缓存所有三角形，再统一光栅、统一 FS（带宽与缓存都扛不住，也不是硬件设计）。

**争用同一位置时的顺序**：不同 draw 的 VS、光栅、FS 可以并行；但当多个片元要读写**同一 attachment 位置**时，GPU 在**输出合并**（Output Merger / ROP）阶段把它们串行化，使深度/模板/混合/写回的结果与 API 提交顺序一致。Early-Z 等优化可能跳过部分 FS，但写回附件的最终语义仍须与此一致。不透明物体之间，**谁可见主要由深度值决定**，不是「后提交的 draw 一定盖住先提交的」——更近的片元通过后仍会写入，即便它来自更晚的 draw。
...
颜色附件和深度附件是显存里的纹理（画布当前纹理 + 深度纹理）。每一个通过测试的片元，在混合完成后**立刻**修改帧缓冲对应像素。因此，在同一 opaque 批次里依次执行各笔 draw 时：

- 草地 draw 写完后，帧缓冲里已经有草地；  
- 人物 draw 在其上覆盖身体像素；  
- 武器 draw 再覆盖手中剑的像素；  
- 换到半透明批次后，法术环与粒子按混合公式叠上去。  

这是「后台画板」上的实时涂改，发生在 GPU 执行 Render Pass 的过程中——注意前三步是**同一批次内的三次 draw**，不是三个独立批次。
...
2. **GPU** 按队列执行；Render Pass 内换 `setPipeline` 即换批次；**同一批次内各次 draw 按录制顺序依次发起**，每笔 draw 仍完整走 VS → 光栅化 → FS → 深度/混合，流水线阶段可重叠但输出合并须与命令顺序一致；草地、人、剑、环、粒子逐步写入显存帧缓冲。  
3. **帧缓冲**随混合实时更新；**屏幕**在整帧呈现时才变。  
4. **Shader** 是管线中的可编程段：VS 管位置与传递属性，FS 管颜色意图；固定阶段负责变成「正确遮挡、正确透明」的像素写入。  
5. **合批**省管线状态切换，不合并 draw 次数；demo 规模下 GPU 利用率低是正常现象。  
6. **UI 与文字**是最后的屏幕空间批次，用图集与字体图集合批绘制，原理与 3D 相同。

把这条链吃透之后，再去看 PBR 公式或噪声流光，都只是某一段 FS/VS 里的细节；而「人物、武器、光效、粒子如何组成一帧」，则始终是调度与管线问题。本地演示 [`webgpu-shader-demo`](https://github.com/shenjunjian/wasm-road-blog/tree/main/webgpu-shader-demo/) 正是按同一条链实现的最小可运行样本。

---

## 8. 纯 WebGPU Shader 源码解读拾遗

前面各章按「原理链」讲渲染；本章反过来，对着 [`webgpu-shader-demo`](https://github.com/shenjunjian/wasm-road-blog/tree/main/webgpu-shader-demo/) 里容易一眼滑过、却又卡人的几处源码细节做拾遗。读时可对照 `src/main.js`、`src/geometry.js`、`src/mesh.js` 与 `src/pipelines/*.js`。
...
若以后要「合批」成一个大缓冲（一次 `drawIndexed` 画多个物体），才需要自己用偏移（`setVertexBuffer(slot, buffer, offset, size)`）在同一块里划区间，或用 Instancing / Multi-draw 喂饱 GPU；当前 demo 故意拆成多次 draw 以便教学，见 §2.2。
...
同一条管线、多份 bind group（不同贴图 / 不同 tint），共用管线状态、分多次 draw——即 §2.2 讲的合批；混合模式或深度写入策略不同，才需要再拆一条 `GPURenderPipeline`（本 demo 的 opaque / aura / particles 就是这样拆的）。
