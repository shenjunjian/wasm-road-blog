...
`@swc/core` 底层是 Rust 编译出的 `.node` 原生模块（N-API），各平台预编译二进制随 npm 包分发。理解 N-API 绑定原理可参见 [`napi-rs-demo/`](https://github.com/shenjunjian/wasm-road-blog/tree/main/napi-rs-demo/)。
...
运行：`cargo run`（完整源码见 [`rust-tools-demo/swc`](https://github.com/shenjunjian/wasm-road-blog/tree/main/rust-tools-demo/swc/)）。
...
- 可运行 demo：[`rust-tools-demo/swc`](https://github.com/shenjunjian/wasm-road-blog/tree/main/rust-tools-demo/swc/)
