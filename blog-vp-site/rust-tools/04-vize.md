...
`vize init` 自动检测 bundler、包管理器、现有 lint 命令，按需安装 `@vizejs/vite-plugin`、`oxlint-plugin-vize`、配置 `vize.config.*` 与 VS Code 扩展推荐。最小可运行示例见 [`rust-tools-demo/vize`](https://github.com/shenjunjian/wasm-road-blog/tree/main/rust-tools-demo/vize/)。
...
这与 [`napi-rs-demo/`](https://github.com/shenjunjian/wasm-road-blog/tree/main/napi-rs-demo/) 的多平台预编译模式一致；浏览器侧参见 [`wasm-road-demo/`](https://github.com/shenjunjian/wasm-road-blog/tree/main/wasm-road-demo/)。
...
- 可运行 demo：[`rust-tools-demo/vize`](https://github.com/shenjunjian/wasm-road-blog/tree/main/rust-tools-demo/vize/)
