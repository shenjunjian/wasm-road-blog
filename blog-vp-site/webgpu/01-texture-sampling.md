...
配套 Demo：[`webgpu-shader-demo/texture/`](https://github.com/shenjunjian/wasm-road-blog/tree/main/webgpu-shader-demo/texture/)（旋转立方体 + 棋盘格，`F`/`A` 切换采样参数）。
...
[`texture/main.js`](https://github.com/shenjunjian/wasm-road-blog/blob/main/webgpu-shader-demo/texture/main.js) 立方体顶点布局：`float32x3` 位置 + `float32x2` UV，与 `VERTEX_LAYOUT` 的 `shaderLocation` 0/1 对应。VS 把 UV 传给 FS，FS 再采样——**几何定义采样坐标，贴图提供颜色**。
...
- Demo 源码：[`texture/shaders/main.wgsl`](https://github.com/shenjunjian/wasm-road-blog/blob/main/webgpu-shader-demo/texture/shaders/main.wgsl)
