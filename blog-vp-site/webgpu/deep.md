...
> 文中行号均指向 `raw/main.js` 等源码文件，建议打开源码对照阅读。
...
本文假设一个最简单的游戏画面：人物站在草地上，手里的武器轻轻摆动，剑身外环绕着流动的法术光环，头顶无数粒子往天上飞。**人物不做运动**，是为了把逻辑减到最少，同时又能看到显卡渲染的完整流程。

全文强调三个概念，请一直带着它们读：

1. **显卡渲染的一切都从三维建模开始。** 以前总觉得游戏里的光影效果很难，不知道怎么做出来的。实际也是先建立顶点（三角形网格），再按透明效果叠一层渲染而已。武器上的金属高光、剑外流动的光环，本质都是"先有几何，再着色"。
...
下面按"初始化 → 建数据 → 绑参数 → 建管线 → 一帧"的顺序，用 `raw/` 的源码走一遍。

---

## 二、初始化设备：拿到 GPU、配置画布（main.js 30–89 行）

### 2.1 三层对象：GPU → Adapter → Device

```js
if (!navigator.gpu) { ... return; }                 // 30
const adapter = await navigator.gpu.requestAdapter();  // 35
const device = await adapter.requestDevice();         // 40
```
...
之后 `canvas.getContext("webgpu")` 拿到画布上下文（45 行）。注意到这里为止，GPU 侧还没有任何大块业务数据——真正占显存的是后面的 buffer 和 texture。

### 2.2 format 与 configure：建立交换链

```js
const format = navigator.gpu.getPreferredCanvasFormat();  // 53
function resize() {
  ...
  canvas.width = w;
  canvas.height = h;
  context.configure({ device, format, alphaMode: "opaque" }); // 62–66
  return { w, h };
}
```

`format` 是**画布颜色纹理的像素格式**，常见 `"bgra8unorm"`（多数 Windows / macOS）或 `"rgba8unorm"`（部分 Linux / Android）。`context.configure()` 做的事比"登记"更多：它让浏览器创建 / 重建一条与画布尺寸匹配的**交换链**，交换链里是一张张可呈现的颜色纹理，每帧由 `getCurrentTexture()` 取出一张来画（第七章细讲）。
...
2. 重新 `configure`，让交换链按新尺寸生成纹理；
3. 重建深度纹理（`recreateDepth()`），因为深度缓冲必须和画布一样大。
...
function recreateDepth() {
  ...
  depthView = depthTexture.createView(); // 87
}
recreateDepth(); // 89
```
...
## 三、创建 Buffer：把几何和常量送进显存（main.js 91–106 行）

场景里的草地、人物、武器、法术环，全部来自 `geometry.js` 生成的 CPU 侧数组（位置 + 法线 + UV 交错排列的 `Float32Array`，加一个 `Uint16Array` 索引）。这些数据在**普通内存**，要画它们，得先搬进显存。`createGpuMesh`（`mesh.js` 26–45 行）干的就是这件事：
...
main.js 91–94 行就是四次调用 `createGpuMesh`，把四个网格上传成四个"顶点缓冲 + 索引缓冲"对。

### 3.1 createBuffer 的两个关键属性
...
### 3.3 uniform buffer：为什么每个物体一个

```js
const UNIFORM_SIZE = 256;                  // 100
...
顺带一提：main.js 96–98 行创建的三条渲染管线（opaque / aura / particles），以及 108–127 行的 BindGroup，是"怎么画、参数从哪来"的定义，第五章与第四章依次展开，这里先跳过。

### 3.4 扩展：createTexture 与 writeTexture
...
## 四、创建 BindGroup：把资源"插"给 shader（main.js 108–127 行）
...
// main.js 108–111 —— 创建 BindGroup 时
...
每帧每个物体的 uniform 内容不同，所以为每个物体建一个"自己的 uniform buffer + 自己的 BindGroup"：`groundBG` / `charBG` / `weaponBG` 分别指向 `groundUB` / `charUB` / `weaponUB`（108–119 行），法术环还有两个（`auraBG` / `auraBG2`）。渲染时换一个 BindGroup，就等于告诉 shader："这组参数换了"。BindGroup 是很轻的对象，它本身不复制数据，只是引用显存里的 buffer。

---

## 五、创建 Pipeline：焊死整条图形管线，并打通 shader 变量（pipelines/opaque.js）
...
- **`vertex`**：描述"顶点从哪来、怎么解析"。`buffers: [VERTEX_LAYOUT]` 就是 `geometry.js` 252–259 行的顶点布局。
...
## 六、帧与命令清单：CPU 编排，GPU 执行（main.js 238–298 行）
...
2. 把结果写进各个 uniform buffer：`fillOpaqueUniform(...)` 内部是 `device.queue.writeBuffer(buffer, 0, data)`（160 行）；
3. 粒子模拟参数同样 `writeBuffer` 进 `simUniform`（213–222 行）。
...
```js
const encoder = device.createCommandEncoder();      // 238
...
// 2) 渲染（Render Pass）
const colorView = context.getCurrentTexture().createView();  // 250
const renderPass = encoder.beginRenderPass({
  colorAttachments: [ ... ],
  depthStencilAttachment: { ... },
});                                              // 251–266

renderPass.setPipeline(opaquePipeline);           // 268 —— 批次开始
...
renderPass.setPipeline(auraPipeline);             // 285 —— 换批次
// ... 两圈法术环（286–291）

renderPass.setPipeline(particles.renderPipeline); // 293
renderPass.draw(6, particles.count);              // 295 实例化绘制

renderPass.end();
device.queue.submit([encoder.finish()]);          // 298
```
...
## 七、补充：fragment / depthStencil 与 RenderPass 附件（main.js 250–266 行）
