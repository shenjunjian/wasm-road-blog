...
0. [第 0 部分：开篇](#第-0-部分开篇)
1. [第 1 章：Wasm 为何需要系统接口 + WASI 应用场景](#第-1-章wasm-为何需要系统接口--wasi-应用场景)
2. [第 2 章：WASI P1 简史与现状](#第-2-章wasi-p1-简史与现状)
3. [第 3 章：Component Model 与 WIT（P2/P3 共用基础）](#第-3-章component-model-与-witp2p3-共用基础)
4. [第 4 章：WASI P2 特性与 API 全景](#第-4-章wasi-p2-特性与-api-全景)
5. [第 5 章：P2 开发方法（Rust + 完整 demo）](#第-5-章p2-开发方法rust--完整-demo)
6. [第 6 章：WASI P3 特性与 API 差异](#第-6-章wasi-p3-特性与-api-差异)
7. [第 7 章：P3 开发方法（概念 + 官方命令，无本地 demo）](#第-7-章p3-开发方法概念--官方命令无本地-demo)
8. [第 8 章：WASI 产物的调用方式](#第-8-章wasi-产物的调用方式)
9. [第 9 章：运行时选型与生态地图](#第-9-章运行时选型与生态地图)
10. [第 10 章：总结与路线图](#第-10-章总结与路线图)
11. [附录 A：WIT 速查](#附录-awit-速查)
12. [附录 B：命令速查](#附录-b命令速查)
13. [附录 C：常见问题与排错](#附录-c常见问题与排错)
14. [附录 D：demo 索引](#附录-ddemo-索引)
15. [附录 E：开源项目速查](#附录-e开源项目速查)
...
| 第 3–5 章 P2 | **主干**：Component/WIT、API、Rust 开发 | `wasi-p2-cli-demo` |
...
P2 与 P3 共享同一套 **Component Model** 与 **WIT** 描述语言；读懂本章，再读 P2 API 与 P3 差异会轻松很多。

### 3.1 Module vs Component

| 维度 | Core Module（P1） | Component（P2/P3） |
|------|-------------------|---------------------|
| 二进制格式 | 单一 `.wasm` 模块 | Component 信封，可内嵌多个模块 |
| 链接方式 | 静态 import/export 名 | WIT 接口 + Canonical ABI |
...
WIT（Wasm Interface Type）是描述 Component 接口的 IDL。核心语法：
...
// World：描述一个 Component 的 import/export 清单
world hello-world {
  export greeter;           // 本组件向外提供
  import wasi:cli/environment@0.2.0;  // 本组件需要宿主提供
}
```
...
WASI 标准接口以**带版本的包名**发布，例如：

- `wasi:cli/command@0.2.0` — P2 命令行程序 world
- `wasi:http/proxy@0.2.0` — P2 HTTP 代理 world
- `wasi:cli/command@0.3.0` — P3 命令行 world（stdio 改为 async stream）
...
选择 **world = 选择程序的「角色」**：CLI 工具选 `command`，HTTP 中间件选 `http/proxy`，纯 HTTP 服务选 `http/service`（P3 拆分后）。

### 3.4 工具链地图

| 工具 | 用途 |
|------|------|
| `wit-bindgen` | 从 WIT 生成 Guest/Host 语言绑定 |
| `wasm-tools` | `component new`、`compose`、`wit` 子命令 |
| `wasi` crate（Rust） | P2/P3 标准库的 Rust 绑定 |
| `cargo-component` | 历史工具，已被 `wasm32-wasip2` target 内置 Component 产出取代 |
| `wkg` | Component 的 OCI 包发布/拉取（Spin 3.0+ 生态） |

Rust 开发者典型路径：`rustup target add wasm32-wasip2` → 普通 `cargo build` → 产物**已是 Component**，无需手动 `component new`（与 P1 不同）。

### 3.5 产物形态

| 阶段 | 产物 | 说明 |
|------|------|------|
| 构建期 | `.wit` 文件、bindgen 生成代码 | 仅开发时使用，不随二进制分发 |
| 运行期 | `.wasm` Component 二进制 | 自包含类型信息与链接元数据 |
| 部署期 | OCI 镜像 / Warg 包（可选） | Spin、wkg 等分发渠道 |

运行时只需：**Component 二进制 + Host 能力配置**（preopen、网络等），不需要在服务器上放置 `.wit` 文件。

**本章小结**：Component = 可组合的二进制包；WIT = 接口契约；World = 角色模板。P2/P3 的所有 API 都以 WIT 包形式组织，下面逐包展开 P2。
...
`wasi-p2-cli-demo` 的 `Cargo.toml` 极为精简——无需 `wit-bindgen` 手动配置，标准库在 `wasm32-wasip2` 上已对接 WASI P2：
...
此文件**已是 Wasm Component**（可用 `wasm-tools component wit` 查看其 world 与 import 清单），无需再执行 `component new`。
...
3. **框架路径**：Spin 3.0+ 可用 `spin build` 直接产出 Component，用 `wkg` 发布到 OCI registry（详见 [Spin 文档](https://spinframework.dev/)）
