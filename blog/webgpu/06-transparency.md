...
[`raw` 综合 demo](../../webgpu-shader-demo/raw/) 已用 **不透明批次 → 半透明批次** 绘制法术环与粒子。本文聚焦半透明 **为何要先画 opaque、关 depth write**，以及 **多透明面交叉** 时为什么要按距离排序。

配套 Demo：[`webgpu-shader-demo/transparency/`](../../webgpu-shader-demo/transparency/)（`S` 切换错误顺序 / 正确排序）。
