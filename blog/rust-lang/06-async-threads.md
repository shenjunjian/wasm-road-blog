...
> 上一篇：[Option 与 Result](./05-option-result.md) · 返回：[系列导读](./README.md)
...
至此本系列六章结束。可从 [导读](./README.md) 回看索引，或按需跳回 [模块与 Cargo](./01-modules-cargo.md)、[集合](./02-collections-structs.md)、[指针](./03-pointers.md)、[Trait](./04-traits.md)、[Option/Result](./05-option-result.md) 查阅。
